...
class outstanding_report_wizard(models.TransientModel):
	...
	def onchange_enddate(self):
		if self.start_date and self.end_date:
			...
			if self.start_date > self.end_date:
				raise ValidationError('Invalid Action!', 'From date cannot be greater that to date.')
	...
	def print_outstanding_report(self):
		...
		if self._context.get('type') == 'pdf':
			...
			return self.env.ref('insabhi_due_statement.report_outstanding_print').report_action(self,data=datas)
		...
		fp = io.BytesIO()
		workbook.save(fp)
		export_id = self.env['outstanding.report.excel'].create({'excel_file': base64.encodebytes(fp.getvalue()), 'file_name': filename})
		fp.close()
		res = {
			'view_mode': 'form',
			'res_id': export_id.id,
